...
(async () => {
  ...
  wsUrl.value = (await kvGet<string>('wsUrl')) || 'http://localhost:7878';
  transport.value = (await kvGet<'wasm' | 'ws'>('transport')) ?? 'ws';
  await kvSet('wsUrl', wsUrl.value);
  ...
  async function bootTransport(): Promise<void> {
    ...
    try {
      ...
      if (want === 'ws') {
        pushLog('warn', `WS transport failed (${msg}) — falling back to WASM`);
        try {
          const c = new WasmClient();
          const info = await c.boot();
          activeClient = c;
          ...
          wireClient(c);
          setClient(activeClient);
          pushLog('ok', `transport WASM (fallback) · nvsim@${info.buildVersion}`);
          return;
        } catch (e2) {
          pushLog('err', `WASM fallback also failed: ${(e2 as Error).message}`);
        }
      }
